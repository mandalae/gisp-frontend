...
class FolderSection extends React.Component {
  ...
  fetchFolders(folder, subFolder) {
    ...
    if (!this.state.subFolders[folder.folderKey]) {
      FolderService.fetchFolders(folder.folderKey)
        .then((folders) => {
          let stateObject = {
            folder: folder,
          };
          ...
          sessionStorage.setItem(
            "covid.previousFolder",
            this.state.previousFolder
              ? this.state.previousFolder.folderName
              : ""
          );
    ...
    if (!sessionUtils.isLoggedIn()) {
      ...
      let items = this.state.folders.map((item, key) => {
        if (item.folderName) {
          if (!this.state.subFolders[item.folderKey]) {
            return (
              <li className="nav-item" key={item.folderKey}>
                <a
                  className={
                    "nav-link btn btn-sm ml-2 mb-1 text-left" +
                    (this.state.folder.folderKey === item.folderKey
                      ? " btn-info"
                      : " btn-outline-info")
                  }
                  href="/documents"
            ...
            const subFolders = this.state.subFolders[item.folderKey].map(
              (subItem, subKey) => {
                return (
                  <li
                    className="nav-item subfolder"
                    key={"sub-" + subItem.folderKey}
                  >
                    <a
                      className={
                        "nav-link btn btn-sm ml-3 mb-1 text-left" +
                        (this.state.folder.folderKey === subItem.folderKey
                          ? " btn-secondary"
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        this.fetchFolders(subItem, true);
                      }}
                    >
                      {subItem.folderName.replace("/", "")}
                    </a>
                  </li>
                );
              }
      ...
      return (
        <div className="sidebar-container">
          <button value="" className="btn btn-primary hide">
